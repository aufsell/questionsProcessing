...
from bot.db_api import check_user_existence # noqa
from bot.keyboards.keyboard import contact_keyboard, approve_register # noqa
...
async def start_common(msg_or_call, state):
    ...
    if await check_user_existence(user_id):
        ...
        await msg_or_call.answer(
            'Привет! Давай зарегистрируемся! Напиши свое имя:',
            )
...
async def last_name(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        'Теперь напиши свой номер телефона',
